...
class DatabaseTest
{
    ...
    public function testBuildQuery(): void
    {
        ...
        // Modified test for nested condition blocks
        #OPTIMIZE: Условные блоки не могут быть вложенными.
        foreach ([null, true] as $block) {
            $results[] = $this->db->buildQuery(
                'SELECT name FROM users WHERE ?# IN (?a){ AND block = ?d { AND user_id = ?d}}',
                ['user_id', [1, 2, 3], $block ?? $this->db->skip(), 2]
            );
        }
        ...
        foreach ($results as $i => $result) {
            echo "<code style='display:block; background-color: #eee; padding: 4px 8px; font-size: 15px;'>" . $result. "</code><br>";
            echo "<code style='display:block; background-color: #eee; padding: 4px 8px; font-size: 15px;'>" . $correct[$i]. "</code><hr>";
        }
